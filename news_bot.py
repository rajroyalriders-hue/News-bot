...
def get_news_hash(title):
    # Normalize — remove numbers, dates, special chars for better dedup
    import re
    clean = title.lower().strip()
    clean = re.sub(r'[0-9]', '', clean)       # remove numbers
    clean = re.sub(r'[^a-z\s]', '', clean)    # keep only letters
    clean = re.sub(r'\s+', ' ', clean).strip() # normalize spaces
    # Use first 60 chars for matching — catches similar headlines
